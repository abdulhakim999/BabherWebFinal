...
const ContentCard: React.FC<ContentCardProps> = ({ item, showImage = true }) => {
  ...
  return (
    <div className={`group bg-white dark:bg-gray-800 rounded-2xl overflow-hidden border border-gray-100 dark:border-gray-700 transition-all duration-300 hover:shadow-xl hover:-translate-y-1 h-full flex flex-col ${isActive ? 'ring-2 ring-amber-500 shadow-md' : ''}`}>
      {/* Image Container */}
      {showImage && item.image && (
        <div className="relative h-48 overflow-hidden bg-gray-100 dark:bg-gray-900">
          <img 
            src={item.image} 
            alt={item.title} 
            className={`w-full h-full object-cover transition-transform duration-700 group-hover:scale-110 ${isActive ? 'scale-105' : ''}`}
            loading="lazy"
          />
          
          {/* Overlay Gradient */}
          <div className={`absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-60 group-hover:opacity-80 transition-opacity ${isActive ? 'opacity-80' : ''}`}></div>
          ...
          <div className="absolute bottom-3 right-3">
             <span className={`text-xs px-3 py-1 rounded-full font-bold shadow-sm ${
               item.type === 'Lesson' ? 'bg-blue-600/90 text-white' :
