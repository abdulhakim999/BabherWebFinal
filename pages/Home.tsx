...
const StatCard = ({ label, count, icon, delay }: { label: string, count: number, icon: React.ReactNode, delay: number }) => (
  <div className={`relative group p-6 rounded-2xl bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 hover:border-amber-500/30 transition-all duration-500 hover:shadow-2xl hover:-translate-y-2 overflow-hidden animate-fade-in-up delay-${delay}`}>
    ...
    <div className="relative z-10 flex flex-col items-center text-center">
      <div className="mb-4 p-4 bg-amber-50 dark:bg-amber-900/20 text-amber-600 dark:text-amber-500 rounded-2xl group-hover:scale-110 group-hover:rotate-3 transition-all duration-300 shadow-sm">
        {icon}
      </div>
...
const CategoryCard = ({ title, count, icon, href, delay }: { title: string, count: string, icon: React.ReactNode, href: string, delay: number }) => (
  <a href={href} className={`block group relative overflow-hidden rounded-2xl h-48 animate-fade-in-up`} style={{ animationDelay: `${delay}ms` }}>
    <div className="absolute inset-0 bg-gradient-to-br from-gray-900 to-gray-800 z-0"></div>
    <div className="absolute inset-0 bg-amber-600/20 group-hover:bg-amber-600/30 transition-colors z-10"></div>
    
    {/* Pattern Overlay */}
    <div className="absolute inset-0 opacity-10 bg-[url('https://www.transparenttextures.com/patterns/arabesque.png')] z-10 transform scale-100 group-hover:scale-110 transition-transform duration-1000"></div>
    
    <div className="absolute inset-0 p-6 flex flex-col justify-between z-20">
      <div className="self-end text-white/80 group-hover:text-amber-300 transition-colors transform group-hover:scale-110 duration-300">
        {icon}
      </div>
      <div>
        <h3 className="text-2xl font-bold text-white font-traditional mb-1 transform translate-y-0 group-hover:-translate-y-1 transition-transform">{title}</h3>
        <p className="text-white/60 text-sm opacity-0 group-hover:opacity-100 transform translate-y-2 group-hover:translate-y-0 transition-all duration-300">{count} مادة علمية</p>
      </div>
    </div>
    
    {/* Arrow Icon */}
    <div className="absolute bottom-4 left-4 w-8 h-8 bg-white/10 rounded-full flex items-center justify-center text-white opacity-0 group-hover:opacity-100 transform translate-x-4 group-hover:translate-x-0 transition-all duration-300">
      <ChevronLeft size={16} />
    </div>
  </a>
);
    ...
    <div className="overflow-hidden">
      ...
      <section className="relative min-h-[90vh] flex items-center justify-center overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gray-900 z-0">
          <div className="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1590076215667-874d4165c643?q=80&w=2835&auto=format&fit=crop')] bg-cover bg-center opacity-30 animate-scale-up" style={{ animationDuration: '20s' }}></div>
        ...
        <div className="container mx-auto px-4 relative z-10 text-center pt-20">
          <div className="max-w-4xl mx-auto space-y-8">
            <div className="inline-block px-4 py-2 bg-amber-500/10 border border-amber-500/20 rounded-full text-amber-400 text-sm font-medium mb-4 animate-fade-in-down">
              الموقع الرسمي
            </div>
            
            <h1 className="text-5xl md:text-7xl font-bold font-traditional text-white leading-tight drop-shadow-2xl animate-fade-in-up delay-100">
              فضيلة الشيخ <br />
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-4 animate-fade-in-up delay-300">
              ...
              <a href="/cv" className="min-w-[160px] px-8 py-4 bg-white/5 hover:bg-white/10 backdrop-blur-md text-white border border-white/10 rounded-xl font-bold transition-all hover:bg-white/20">
                السيرة الذاتية
              </a>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
            ...
            <div className="order-1 md:order-2 relative animate-scale-up delay-200">
              <div className="relative z-10 rounded-3xl overflow-hidden shadow-2xl rotate-2 hover:rotate-0 transition-transform duration-500">
                <div className="aspect-[4/5] bg-gray-200 dark:bg-gray-800 relative">
